...
const [data,setData] = useState({
    ...
    return (
        <div className="flex flex-col px-2 py-4 items-center mt-10">
            ...
            <div className="flex items-center mt-2 mb-3  w-25 bg-zinc-500 p-2 rounded-md "> <span className="crown">{<AiFillCrown/>}</span>boostmysteez.io/<span className="page-link" placeholder="Nickname" readOnly>{data.username}</span></div>
            <div className="bg-gradient-to-r from-purple-200 to-amber-200 ... bg-opacity-60 backdrop-filter backdrop-blur-lg  px-5 py-3 rounded-md">
              <form className=" flex flex-col " onSubmit={signupUser}>
                ...
                <div className="mt-2 flex justify-center w-full text-black">Already Signed Up? <span className="font-bold ml-2 cursor-pointer" onClick={() => {setAuthScreen("Login")}}>Login</span></div>
              </form>
            </div>
        </div>
    );
